...
import { DirectStopMarketOrderDto, TradeSide, OrderExecutionResponseDto } from '@/dtos/order.dto';

@Injectable()
export class StopMarketOrderUseCase {
  private readonly logger = new Logger(StopMarketOrderUseCase.name);
  ...
  async execute(userAddress: string, orderData: DirectStopMarketOrderDto): Promise<OrderExecutionResponseDto> {
    try {
      this.logger.log(`Executing stop market order for user: ${userAddress}`);
      ...
      const orderDetail = {
        ...
        p: orderData.stopPrice.toString(),
      ...
      this.logger.error(`Failed to execute stop market order:`, error);
