...
import { DirectLimitOrderDto, TradeSide, OrderExecutionResponseDto } from '@/dtos/order.dto';

@Injectable()
export class LimitOrderUseCase {
  private readonly logger = new Logger(LimitOrderUseCase.name);
  ...
  async execute(userAddress: string, orderData: DirectLimitOrderDto): Promise<OrderExecutionResponseDto> {
    try {
      this.logger.log(`Executing limit order for user: ${userAddress}`);
      ...
      this.logger.error(`Failed to execute limit order:`, error);
